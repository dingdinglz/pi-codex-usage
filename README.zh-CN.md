...
- **后台刷新：** 默认每 5 分钟查询，agent 运行结束后限频更新，也支持手动刷新。
...
- 默认每 **5 分钟**查询一次。agent 运行结束后也会尝试更新，但自动请求间隔不少于 **60 秒**。
- 手动刷新至少间隔 **5 秒**；同一实例中的并发请求合并。
- 失败会退避 **5–30 分钟**。HTTP 429 的 `Retry-After` 对同一扩展实例中的手动刷新、provider 切换及 off/on 同样有效。
